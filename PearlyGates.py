...
class PearlyGates:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://www.pearlygates.net/shoplist/"
    ...
    def getStoreInfo(self):
        ...
        store_info = []
        areas =['hokkaido_tohoku','hokuriku_koshinetsu','kanto','tokyo','tokai','kinki','chugoku_shikoku','kyusyu','outlet']
        
        for html in html_contents:
            soup = BeautifulSoup(html, "html.parser")
            shoplist = soup.find('div',{'class':'shoplist'})
            
            for area in areas:
                stores = shoplist.find('li',{'id':area}).find_all('li',{'class':'panel'})

                for store in stores:
                    if store.find('p',{'class':'add'}).span:
                        store.find('p',{'class':'add'}).span.decompose()
                    name = store.find('p',{'class':'shopName'}).text.strip()
                    address = store.find('p',{'class':'add'}).text.strip()
...
if __name__ == '__main__':
    try:
        ...
        df = PearlyGates(PearlyGates.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/PearlyGates.csv', index=True)
