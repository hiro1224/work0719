...
class JeansStation:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://jeans-station.com/f/shoplist"
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            for store in stores:
                name = store.find('h2',{'class':'shoplist-title'}).text
                address = store.find('dl',{'class':'fs-c-explainList'}).find('dd').text
...
if __name__ == '__main__':
    try:
        ...
        df = JeansStation(JeansStation.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/JeansStation.csv', index=True)
