...
class PaulSmith:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://www.paulsmith.co.jp/shop-locator"
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            stores = soup.find('div',{'class':'stores_wrapper'}).find_all('div',{'class':'story-card-block'})

            for store in stores:
                name = store.find('h2',{'class':'story-card-title'}).text.replace('\n','')
                address = store.find_all('span',{'class':'store_address'})[0].text + ' ' + store.find_all('span',{'class':'store_address'})[1].text
...
if __name__ == '__main__':
    try:
        ...
        df = PaulSmith(PaulSmith.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/PaulSmith.csv', index=True)
