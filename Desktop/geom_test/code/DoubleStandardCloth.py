...
class DoubleStandardCloth:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "http://doublestandard.jp/store-list/"
    ...
    def getHtmlData(self, url_list):
        ...
        try:
            for url in url_list:
                ...
                btn_elems = driver.find_elements_by_class_name('accordion-title')
                scroll_down(driver)
                time.sleep(1)
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            areas = soup.find_all('li',{'class':'current'})
            
            for area in areas:
                area_name = area.find('div',{'class':'accordion-title'}).text.replace(' ','')
                info = area.find_all('p')

                for i in range(int(len(info)/3)):
                    name = area_name + ' ' + info[i*3].text.strip()
                    address = info[i*3+1].text.split('\n')[0].strip()
                    end_index = address.find('電話番号')
                    if end_index != -1:
                        address = address[:end_index].strip()
...
if __name__ == '__main__':
    try:
        ...
        df = DoubleStandardCloth(DoubleStandardCloth.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/DoubleStandardCloth.csv', index=True)
