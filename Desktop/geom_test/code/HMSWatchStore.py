...
class HMSWatchStore:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://www.hms-watchstore.com/shoplist/"
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            flagship_info = soup.find('div',{'class':'swiper_right'})
            name = flagship_info.find('p',{'class':'fs_txt'}).text
            info = flagship_info.find('p',{'class':'en_info'}).text.split('\n')
            address = info[0][7:] + ' ' + info[1].strip()
            store_info.append((name, address))
            stores = soup.find('div',{'class':'page_content'}).find('ul').find_all('li')
            
            for store in stores:
                name = store.find('h3').text.strip()
                address = store.find_all('p')[-1].text.split('\n')[0].strip()
                store_info.append((name, address))
...
if __name__ == '__main__':
    try:
        ...
        df = HMSWatchStore(HMSWatchStore.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/HMSWatchStore.csv', index=True)
