...
class MichaelKors:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://www.michaelkors.jp/storelocator/"
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            stores = soup.find('div',{'id':'allShopList'}).find_all('div',{'style':'display: block;'})
            
            for store in stores:
                name = store.find('p',{'class':'tit'}).text.strip()
                address = store.find('p',{'class':'text text02'}).text.strip()
...
if __name__ == '__main__':
    try:
        ...
        df = MichaelKors(MichaelKors.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/MichaelKors.csv', index=True)
