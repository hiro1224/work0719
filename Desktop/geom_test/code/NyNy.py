...
class NyNy:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://www.nyny.co.jp/salon/"
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            areas = soup.find('div',{'id':'salonListMainArea'}).find_all('section',{'class':'clearfix'})
            
            for area in areas:
                stores = area.find('ul',{'class':'salonList'}).find_all('li',{'class':'salon'})
                
                for store in stores:
                    info = store.find('div',{'class':'textBox'})
                    name = info.find('h4').text.strip()
                    address = info.find('p',{'class':'address'}).text.strip()
...
if __name__ == '__main__':
    try:
        ...
        df = NyNy(NyNy.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/NyNy.csv', index=True)
