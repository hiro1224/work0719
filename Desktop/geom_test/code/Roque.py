...
class Roque:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://www.roque.jp/html/page11.html"
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            for store in stores:
                name = store.find('div',{'class':'headline'}).text.strip()
                info = store.find_all('div',{'class':'shopguide-right'})[1].text
                start_index = info.find('店') + 1
                end_index = info.find('TEL')
                if (start_index != -1) and (end_index != -1):
                    address = ' '.join(info[start_index:end_index].split())
                else:
                    address = ' '.join(info.split())
...
if __name__ == '__main__':
    try:
        ...
        df = Roque(Roque.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/Roque.csv', index=True)
