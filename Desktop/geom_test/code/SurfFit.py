...
class SurfFit:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://surffit.jp/studio/"
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            stores = soup.find('ul',{'class':'studioList'}).find_all('li',{'class':'studioList-item'})
            
            for store in stores:
                name = store.find('dt').text.replace('-','')
...
if __name__ == '__main__':
    try:
        ...
        df = SurfFit(SurfFit.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/SurfFit.csv', index=True)
