...
class OnitsukaTiger:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://www.onitsukatiger.com/jp/ja-jp/store-finder?page="
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            stores = soup.find('div',{'class':'store-list'}).find_all('li',{'class':'store-item'})
            
            for store in stores:
                name = store.find('h4').text.strip()
                address = store.find('span',{'class':'street-address'}).text.strip()
...
if __name__ == '__main__':
    try:
        ...
        df = OnitsukaTiger(OnitsukaTiger.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/OnitsukaTiger.csv', index=True)
