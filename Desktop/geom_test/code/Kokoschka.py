...
class Kokoschka:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://www.jfg-inc.com/kokoschka/#shoplist"
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            stores = soup.find('div',{'id':'shoplist'}).find_all('li')
            for store in stores:
                name = store.find('a').text.strip()
                address = store.find('p').find('a').text.strip()
...
if __name__ == '__main__':
    try:
        ...
        df = Kokoschka(Kokoschka.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/.csv', index=True)
