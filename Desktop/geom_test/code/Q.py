...
class Q:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://www.qshop.jp/store"
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            stores = soup.find('div',{'id':'wrap'}).find_all('p')
            for i in range(len(stores)):
                if stores[i].text[0] == '〒':
                    name = stores[i-1].text.strip()
                    address = stores[i].text.strip()
...
if __name__ == '__main__':
    ...
    # df.to_csv('~/Desktop/geom_test/csv/Q.csv', index=True)
