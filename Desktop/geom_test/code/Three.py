...
class Three:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://www.threecosmetics.com/brand/shop_type/counseling-shop/"
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            stores = soup.find('article',{'class':'detail'}).find_all('dl',{'class':'shop_list'})
            for store in stores:
                if store.find('a').div:
                    store.find('a').div.decompose()
                name = store.find('a').text.strip()
                address = store.find('dd').text
                start_index = address.find('〒')
                end_index = address.find('TEL')
                if address.find('定休日') != -1:
                    end_index = address.find('定休日')
                address = address[start_index:end_index].strip().replace('\n','')
...
if __name__ == '__main__':
    try:
        ...
        df = Three(Three.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/Three.csv', index=True)
