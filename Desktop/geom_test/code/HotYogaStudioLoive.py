...
class HotYogaStudioLoive:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://www.hotyoga-loive.com/studio/"
    ...
    def getHtmlData(self, url):
        ...
        try:
            ...
            areas = soup.find('section',{'class':'parentBox'}).find_all('section',{'class':'areaBox'})
            
            for area in areas:
                stores = area.find('ol',{'class':'clear'}).find_all('li')
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            top = soup.find('h1',{'class':'studio_top'})
            
            if top.span:
                top.span.decompose()
                
            name = top.text.strip()
            address = soup.find('ol',{'class':'access-deta'}).find('dd').text.split('                         ')[1]
...
if __name__ == '__main__':
    try:
        ...
        df = HotYogaStudioLoive(HotYogaStudioLoive.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/HotYogaStudioLoive.csv', index=True)
