...
class StyleLeatherCraft:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://style-lc.jp/shop-list"
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            for table in tables:
                stores = table.find('tbody').find_all('tr')
                for store in stores:
...
if __name__ == '__main__':
    try:
        ...
        df = StyleLeatherCraft(StyleLeatherCraft.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/StyleLeatherCraft.csv', index=True)
