...
class CocoManna:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://manna-japan.jp/shop_cocomanna.html"
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            info_list = soup.find('section',{'class':''}).find_all('p')
            
            for i in range(len(info_list)):
                if (i%3 == 2) and (info_list[i].text[0] == '〒'):
                    name = info_list[i-1].text.strip()
                    address = info_list[i].text.strip()
...
if __name__ == '__main__':
    try:
        ...
        df = CocoManna(CocoManna.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/CocoManna.csv', index=True)
