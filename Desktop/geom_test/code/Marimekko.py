...
class Marimekko:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://www.marimekko.jp/shops/"
    ...
    def getHtmlData(self, url):
        ...
        html_contents = []
        store_url_list = []
        area_list = ['kanto','tohoku','kousinetu','tokai','chugoku','kyusyu']
        try:
            for area in area_list:
                ...
                stores = driver.find_element_by_class_name('shops-inner-wrap').find_elements_by_tag_name('li')[:-1]
                
                for store in stores:
                    store_url = store.find_element_by_tag_name('a').get_attribute("href")
                    store_url_list.append(store_url)
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            name = soup.find('div',{'class':'store_detail'}).find('h2').text.strip()
            address = soup.find('div',{'class':'store_detail'}).find('div',{'class':'detail_caption'})
            if address.h3:
                address.h3.decompose()
            address = ' '.join(address.text.split())
            end_index = address.find('Tel')
            address = address[:end_index]
...
if __name__ == '__main__':
    try:
        ...
        df = Marimekko(Marimekko.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/Marimekko.csv', index=True)
