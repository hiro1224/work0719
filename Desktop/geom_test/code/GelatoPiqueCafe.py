...
class GelatoPiqueCafe:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://pique-cafe.com/shop.html"
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            stores = soup.find('section',{'class':'content_space'}).find_all('dl')
            for store in stores:
                name = store.find('dt').text.strip()
                address = store.find('dd',{'class':'info_1'}).text.strip()
...
if __name__ == '__main__':
    try:
        ...
        df = GelatoPiqueCafe(GelatoPiqueCafe.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/GelatoPiqueCafe.csv', index=True)
