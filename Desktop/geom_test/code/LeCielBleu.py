...
class LeCielBleu:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://www.lcb.co.jp/stores/"
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            stores = soup.find('div',{'class':'page'}).find_all('li',{'class':'shoplist-box'})
            
            for store in stores:
                name = store.find('h4',{'class':'heading'}).text.strip()
...
if __name__ == '__main__':
    try:
        ...
        df = LeCielBleu(LeCielBleu.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/LeCielBleu.csv', index=True)
