...
class Ravijour:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://www.ravijour.com/guide/shoplist/"
    ...
    def getHtmlData(self, url):
        ...
        try:
            ...
            areas = driver.find_element_by_class_name('InnerContent').find_elements_by_class_name('shop')
            
            for area in areas:
                stores = area.find_elements_by_tag_name('a')
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            store = soup.find('dl',{'class':'list_shop'})
            if store.find('dt').span:
                store.find('dt').span.decompose()
            name = store.find('dt').text.strip()
            address = store.find('dd').text.strip().split('\n')[0]
...
if __name__ == '__main__':
    try:
        ...
        df = Ravijour(Ravijour.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/Ravijour.csv', index=True)
