...
class Train:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://www.train-ing.jp/"
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            stores_name = soup.find('div',{'id':'access_list_sp'}).find_all('p',{'class':'shop_title'})
            stores_address = soup.find('div',{'id':'access_list_sp'}).find_all('p',{'class':'shop_info'})
            for i in range(len(stores_name)):
                name = stores_name[i].text
                address = stores_address[i].text.split('\n')[0]
...
if __name__ == '__main__':
    try:
        ...
        df = Train(Train.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/Train.csv', index=True)
