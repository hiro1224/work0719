...
class PilatesK:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "https://pilates-k.jp/studio"
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            stores = soup.find('section',{'id':'shopbox'}).find_all('div',{'class':'box'})
            
            for store in stores:
                if store.find('h3').span:
                    store.find('h3').span.decompose()
                name = store.find('h3').text
                address = ''.join(store.find('p').text.split('\n'))
...
if __name__ == '__main__':
    try:
        ...
        df = PilatesK(PilatesK.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/PilatesK.csv', index=True)
