...
class LogosShop:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = 'https://www.logos.ne.jp/shop/search?page='
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            stores = soup.find('div',{'class':'shop_search'}).find_all('div',{'class':'item'})
            
            for store in stores:
                icon = store.find('span',{'class':'shop_icon'}).text
                
                if (icon == '直営店') or (icon == 'アウトレット'):
                    name = store.find('h3',{'class':'midashigo'}).text.strip()
                    address = ' '.join(store.find('div',{'class':'shop_info'}).find_all('td')[0].text.split('\n'))
...
if __name__ == '__main__':
    try:
        ...
        df = LogosShop(LogosShop.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/LogosShop.csv', index=True)
