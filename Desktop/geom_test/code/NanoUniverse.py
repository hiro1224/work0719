...
class NanoUniverse:  # 'Brand'には対象のブランド名をキャメルケースで入れてください
    BASE_URL = "http://www.nanouniverse.jp/"
    ...
    def getHtmlData(self, url):
        ...
        try:
            ...
            areas = soup.find('div',{'id':'index22'}).find_all('div',{'class':'index22c'})
            
            for area in areas:
                stores = area.find('div',{'class':'regular'}).find_all('li') + area.find('div',{'class':'outlet'}).find_all('li')
                
                for store in stores:
                    url_list.append(url[0] + store.find('a').get("href"))
            
            for url_store in url_list:
    ...
    def getStoreInfo(self):
        ...
        for html in html_contents:
            ...
            store = soup.find('section',{'id':'shop1'})
            name = store.find('h2',{'id':'shop11'}).text.strip()
            address = store.find('p',{'id':'shop12'}).text.split('\n')[0].strip()
...
if __name__ == '__main__':
    try:
        ...
        df = NanoUniverse(NanoUniverse.BASE_URL).getStoreInfo()
    ...
    # df.to_csv('~/Desktop/geom_test/csv/NanoUniverse.csv', index=True)
